    ...
    <div id="blogList">
      <el-card class="box-card">
        ...
        >
          ...
          <div class="configbar">
            <i class="el-icon-date" /> 发表于{{
              new Date(item.created).format("yyyy-MM-dd hh:mm:ss")
            }}
            | <i class="el-icon-s-flag" />阅读{{ item.visitCount }} |
            <i class="el-icon-date" />上次更新:{{
              new Date(item.updated).format("yyyy-MM-dd hh:mm:ss")
            }}
            | <i class="el-icon-medal" />喜欢:{{ item.likes }}
          </div>
          <div class="BlogContent" v-html="contentFormatSub(item)"></div>
...
export default {
  ...
  methods: {
    ...
    contentFormatSub(item) {
      ...
      return marked(item.body).substring(0, 300);
    },
    async getCreateDate() {
